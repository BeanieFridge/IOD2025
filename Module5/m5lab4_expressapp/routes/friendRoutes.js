...
router.get('/filter', (req, res) => {
    ...
    if (matchingFriends.length > 0) {
        ...
        res.status(404).json({error: `No friends matching gender ${filterGender} and no matching letter ${filterLetter}`})
    }  
})
...
router.get('/:id', (req, res) => {
    ...
    friend ? res.status(200).json({result: friend}) : res.status(404).json({result: `Friend with ID:${friendId} does not exist`})
})
